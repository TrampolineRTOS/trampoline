...
We need to make room (16 bytes) on the stack for volatile registers. It is not necessary to save them since the caller does not expect their values to be preserved. The two 16 bits words at the bottom of this area is for the \lstinline{REG_RETARG} which is not saved yet.
...
\draw[thick] (0,5) rectangle ++(16,1); \node at ($(8,5.5)$) {space of arg - will hold {\small REG\_RETARG} [15..0]};

\draw[thick] (0,4) rectangle ++(16,1); \node at ($(8,4.5)$) {space of arg - will hold {\small REG\_RETARG} [19..16]};
...
In case of stack switch, the process stack pointer is saved on the kernel stack
...
From there, \lstinline{REG_RETARG} holds the return value. It is put at its location in the process stack.
...
\end{lstlisting}
Save the context. The MSP430 have a "push multiple words", but no "mov multiple word". So, we get back to process stack to benefit this instruction
