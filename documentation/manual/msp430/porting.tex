...
\begin{figure}[h!]
\caption{Context initialization}
\begin{center}
\begin{tikzpicture}[xscale=0.6, yscale=-.6, every node/.style={scale=0.75}]
\foreach \r in {4,5,...,10} {
	\draw[thick] ($(0,\r-4)$) rectangle ++(16,1); \node at ($(8,\r-4+.5)$) {r\r};
...
\section{MCU Clocks}
The MCU clocks is set to \emph{1MHz} at startup (defined in \lstinline{tpl_startup.c}). It uses the DCO as input clock, and a user fonction is provided to update the frequency in \lstinline{tpl_clocks.h}:
