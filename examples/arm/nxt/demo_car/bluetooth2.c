...
#include "tpl_memmap.h"
void incresaseSpeed(int a){
	
	if (vitesse <= (100 - a ))  vitesse = vitesse + a;
...
void sendCommand(char commande){
	  
	switch(commande){
		case 1:
			if (vitesse >= 0 ){
				incresaseSpeed(5);
				nxt_motor_set_speed(NXT_PORT_A,vitesse, 0);
				nxt_motor_set_speed(NXT_PORT_B,vitesse, 0);
			}
			else {
				vitesse = 0;
		...
		case 2:
			...
			break;
		case 3:
			nxt_motor_set_count(NXT_PORT_C,45);
			break;
	}
}
...
TASK(task_check_bt)
{   
    static u8 bt_receive_buf[5]; 
    DisableAllInterrupts();
    /* read packet data from the master device */  
    int val = ecrobot_read_bt_packet(bt_receive_buf, 5);
	EnableAllInterrupts();
	if (val > 0) {  
		compteur ++;
		displayValue("val:", val, 0, 7);
    ...
    //{
		...
		displayValue("buf2:", bt_receive_buf[2], 0, 3);
		displayValue("buf3:", bt_receive_buf[3], 0, 4);
			displayValue("buf4:", bt_receive_buf[4], 0, 5);
		displayValue("compteur", compteur, 0, 6);
       // displayValue("buf1:", bt_receive_buf[2], 0, 2);
        display_update();
        ...
        //direction = (s32)((s8)bt_receive_buf[2]);
        
    //}
	ecrobot_send_bt_packet(bt_receive_buf,4);
	sendCommand(bt_receive_buf[0]);
    bt_receive_buf[0] = 0;
