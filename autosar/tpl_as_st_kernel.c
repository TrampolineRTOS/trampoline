...
#endif
{
  ...
  P2VAR(tpl_schedule_table, AUTOMATIC, OS_APPL_DATA)
  next = ((P2VAR(tpl_schedule_table, AUTOMATIC, OS_APPL_DATA))st)->next;

  /*  Get the current index                                               */
  /*  MISRA RULE 45 VIOLATION: a tpl_time_obj* is cast to a
      tpl_schedtable*. This cast behaves correctly because the first member
      of tpl_schedula_table is a tpl_time_obj */
  VAR(tpl_expiry_count, AUTOMATIC)
  ...
  P2VAR(tpl_expiry_point, AUTOMATIC, OS_APPL_DATA)
  current_ep = (schedtable->expiry)[index];
  ...
  VAR(tpl_expiry_count, AUTOMATIC)
  abs_deviation = (~(
      ((P2VAR(tpl_schedule_table, AUTOMATIC, OS_APPL_DATA))st)->deviation - 1));
  ...
  if ((index_temp == (schedtable->count - 1)) &&
      ((schedtable->periodic == FALSE) || (next != NULL)))
  {
    /* do nothing */
  }
  else
  {
    if (index != (uint16)(-1))
    {
      ...
      /* adjust next expiry point if EXPLICIT schedule table and not
       * asynchronous */
      ...
      {
        tpl_adjust_next_expiry_point(st, index,
                             ...
                             VAR(tpl_tick, AUTOMATIC) offset)
{
  ...
  /* MISRA RULE 45 VIOLATION: a tpl_time_obj_static* is cast to a
     ...
     memeber of tpl_schedula_table_static is a tpl_time_obj_static */
  schedtable = (P2VAR(tpl_schedtable_static, AUTOMATIC, OS_APPL_DATA))(
      st->b_desc.stat_part);
                                     ...
                                     VAR(tpl_tick, AUTOMATIC) offset)
{
  ...
  CHECK_SCHEDTABLE_SYNC_STRATEGY_EQUAL_ERROR(sched_table_id,
                                             SCHEDTABLE_IMPLICIT_SYNC, result)
                             ...
                             VAR(tpl_tick, AUTOMATIC) tick_val)
{
  ...
  if (st->b_desc.state == (tpl_schedtable_state)SCHEDULETABLE_STOPPED)
  {
    ...
    /*printf("startstabs - tick_val=%d - (schedtable->expiry[0])->offset=%d -
     * cnt->max_allowed_value=%d -
     * cnt->current_date=%d\n",tick_val,(schedtable->expiry[0])->offset,cnt->max_allowed_value,cnt->current_date);*/
    /* if <tick_val> is after current_date and first expiry point comes between
      current_date and <tick_val> or if <tick_val> is before current_date and
      first expiry point comes after current_date so, bootstrap is needed */
    if (((date > cnt->max_allowed_value) &&
         ((date - (cnt->max_allowed_value + 1)) > cnt->current_date)) ||
                                            ...
                                            OS_APPL_DATA) status)
{
  ...
  IF_NO_EXTENDED_ERROR(result)
  {
    st = tpl_schedtable_table[sched_table_id];
    *status =
        (st->b_desc.state & ~SCHEDULETABLE_BOOTSTRAP & ~SCHEDULETABLE_ASYNC);
  }
#endif
