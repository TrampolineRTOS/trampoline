 ...
 */
FUNC(void, OS_CODE)
tpl_action_increment_counter(P2CONST(tpl_action, AUTOMATIC, OS_APPL_CONST)
                                 action)
{
  /*
 ...
 */
FUNC(void, OS_CODE)
tpl_action_finalize_schedule_table(P2CONST(tpl_action, AUTOMATIC, OS_APPL_CONST)
                                       action)
{
  /*
  ...
  st = &(((P2CONST(tpl_finalize_schedule_table_action, AUTOMATIC,
                   OS_APPL_CONST))action)
             ->schedule_table->b_desc);
  ...
  /*VAR(tpl_expiry_count, AUTOMATIC) abs_deviation = (
  ...
  if (next != NULL)
  {
    ...
    /* if next, the synchronisation if forgotten because it's like a changing of
     * mode. */
    ...
    /*  Get the next expiry point                                        */
    next_ep = ((P2VAR(tpl_schedtable_static, AUTOMATIC,
                      OS_APPL_DATA))next->b_desc.stat_part)
                  ->expiry[0];
    ...
    if (next_ep->offset == 0)
    {
      ...
      for (i = 0; i < next_ep->count; i++)
      {
        ...
        (action_desc->action)(action_desc, FALSE);
#else
    ...
    next->b_desc.date =
        next->b_desc.stat_part->counter->current_date + next_ep->offset;
