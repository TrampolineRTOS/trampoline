...
FUNC(void,OS_CODE) tpl_set_mcu_clock(uint16_t freq)
{
	...
	//When we change the clock settings, we must be sure that the wait states are 
	// - set before increasing the clock speed
	// - removed after decreasing the clock speed
	// We set the wait states to 2 cycles here, and we adapt after for low frequencies.
