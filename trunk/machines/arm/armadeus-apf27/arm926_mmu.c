 ...
 * area. This areas cannot be accessed by an untrusted process.
 */
FUNC(void, OS_CODE) MMU_set_system_area (tpl_task_id this_process, u8 *from, u8 *to)
{
  MMU_set_tiny_pages_area (this_process, from, to, 1);
}
...
{
  MMU_set_tiny_pages_area (this_process, from, to, 2);
}
