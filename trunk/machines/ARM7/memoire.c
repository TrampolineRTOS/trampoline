...
/**
 * @brief Variable fictive dont l'adresse correspond au début de la zone des variables statiques non initialisées
 *
 * Il s'agit en fait d'une étiquette à définir dans le script d'édition des liens.
 */
extern UInt32 debut_bss;

/**
 * @brief Variable fictive dont l'adresse correspond à la fin de la zone des variables statiques non initialisées
 *
 * Il s'agit en fait d'une étiquette à définir dans le script d'édition des liens.
 */
extern UInt32 fin_bss;

extern UInt32 debut_rom;
extern UInt32 fin_rom;
...
{
  UInt32 *rmpcon = (UInt32*)MEMOIRE_RMPCON_REG;
  return (*rmpcon & MEMOIRE_RMPCON_MASQUE_ROM) == MEMOIRE_RMPCON_DRAM;
}
