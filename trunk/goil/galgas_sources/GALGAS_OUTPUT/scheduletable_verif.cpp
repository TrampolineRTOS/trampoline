        ...
        while (((operand_11658 = enumerator_11658.nextObject ()))) {
          ...
          GGS_lstring  var_cas_ep_name = operand_11658->point.reader_name (inLexique COMMA_SOURCE_FILE_AT_LINE (258)) ;
          {
            ...
            while (((operand_11731 = enumerator_11731.nextObject ()))) {
              ...
              { const GGS_action_obj _var_12940 = operand_11731->action ; // CAST instruction
                if (_var_12940.getPtr () != NULL) {
                  macroValidPointer (_var_12940.getPtr ()) ;
                  if (typeid (cPtr_activatetask_action) == typeid (* (_var_12940.getPtr ()))) {
                    const GGS_activatetask_action var_cas_ata (_var_12940.getPtr ()) ;
                    const GGS_bool cond_11997 = (var_cas_cpu.reader_tasks (inLexique COMMA_SOURCE_FILE_AT_LINE (262)).reader_hasKey (var_cas_ata.reader_task_name (inLexique COMMA_SOURCE_FILE_AT_LINE (262)).reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (262)))).operator_not () ;
                    if (cond_11997.isBuiltAndTrue ()) {
                      operand_11731->action.reader_location (inLexique COMMA_SOURCE_FILE_AT_LINE (263)).reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("TASK attribute of action does not reference an existing task") COMMA_SOURCE_FILE_AT_LINE (264)) ;
                    }
                  }else if (typeid (cPtr_setevent_action) == typeid (* (_var_12940.getPtr ()))) {
                    const GGS_setevent_action var_cas_sea (_var_12940.getPtr ()) ;
                    const GGS_bool cond_12225 = (var_cas_cpu.reader_tasks (inLexique COMMA_SOURCE_FILE_AT_LINE (266)).reader_hasKey (var_cas_sea.reader_task_name (inLexique COMMA_SOURCE_FILE_AT_LINE (266)).reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (266)))).operator_not () ;
                    if (cond_12225.isBuiltAndTrue ()) {
                      operand_11731->action.reader_location (inLexique COMMA_SOURCE_FILE_AT_LINE (267)).reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("TASK attribute of action does not reference an existing task") COMMA_SOURCE_FILE_AT_LINE (268)) ;
                    }
                    GGS_task_obj  var_cas_task_data ;
                    GGS_lstringlist  var_cas_known_events ;
                    const GGS_task_map  temp_12318 = var_cas_tasks ;
                    if (temp_12318.isBuilt ()) {
                      temp_12318 (HERE)->method_get (inLexique, var_cas_sea.reader_task_name (inLexique COMMA_SOURCE_FILE_AT_LINE (271)), var_cas_task_data COMMA_SOURCE_FILE_AT_LINE (271)) ;
                    }
                    var_cas_known_events = var_cas_task_data.reader_events (inLexique COMMA_SOURCE_FILE_AT_LINE (272)) ;
                    const GGS_bool cond_12601 = (var_cas_known_events.reader_length ()).operator_isEqual (GGS_uint (0U)) ;
                    if (cond_12601.isBuiltAndTrue ()) {
                    ...
                    const GGS_bool cond_12793 = (var_cas_cpu.reader_events (inLexique COMMA_SOURCE_FILE_AT_LINE (276)).reader_hasKey (var_cas_sea.reader_event_name (inLexique COMMA_SOURCE_FILE_AT_LINE (276)).reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (276)))).operator_not () ;
                    if (cond_12793.isBuiltAndTrue ()) {
                      operand_11731->action.reader_location (inLexique COMMA_SOURCE_FILE_AT_LINE (277)).reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("EVENT attribute of action does not reference an existing event") COMMA_SOURCE_FILE_AT_LINE (278)) ;
                    }
                  }else{
