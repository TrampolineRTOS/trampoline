...
method_generate_c (C_Compiler & _inLexique,
                                GGS_string& var_cas_res COMMA_UNUSED_LOCATION_ARGS) const {
  var_cas_res = ((GGS_string (true, "#define    TRACE_FORMAT   "))._operator_concat (form.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (70))))._operator_concat (GGS_string (true, "\n")) ;
  GGS_string var_cas_m ;
  const GGS_trace_method  _temp_1611 = meth ;
  if (_temp_1611._isBuilt ()) {
    _temp_1611 (HERE)->method_generate_c (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (72)) ;
  }
...
void routine_set_method (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_trace_method _var_2927 = var_cas_t ; // CAST instruction
    if (_var_2927.getPtr () != NULL) {
      macroValidPointer (_var_2927.getPtr ()) ;
      if (typeid (cPtr_trace_void) == typeid (* (_var_2927.getPtr ()))) {
        var_cas_t = var_cas_s ;
      }else{
        var_cas_s.reader_loc (_inLexique COMMA_SOURCE_FILE_AT_LINE (123)).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, (GGS_string (true, "Redefinition of "))._operator_concat (var_cas_att) COMMA_SOURCE_FILE_AT_LINE (124)) ;
        var_cas_t.reader_loc (_inLexique COMMA_SOURCE_FILE_AT_LINE (124)).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "was defined here") COMMA_SOURCE_FILE_AT_LINE (125)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
