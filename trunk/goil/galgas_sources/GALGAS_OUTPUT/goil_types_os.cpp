...
method_generate_c (C_Compiler & _inLexique,
  ...
  const GGS_trace_method  _temp_1718 = meth ;
  if (_temp_1718._isBuilt ()) {
    _temp_1718 (HERE)->method_generate_c (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (73)) ;
  }
...
void routine_set_method (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_trace_method _var_3034 = var_cas_t ; // CAST instruction
    if (_var_3034.getPtr () != NULL) {
      macroValidPointer (_var_3034.getPtr ()) ;
      if (typeid (cPtr_trace_void) == typeid (* (_var_3034.getPtr ()))) {
        var_cas_t = var_cas_s ;
      }else{
