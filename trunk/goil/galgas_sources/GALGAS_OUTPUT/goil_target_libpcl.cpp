...
void routine_generate_target_libpcl (C_Compiler & _inLexique,
  ...
  #endif
  ::routine_doReplace (_inLexique,  var_cas_result,  GGS_string ("$STACK_ZONE$"),  (var_cas_name.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (22)))._operator_concat (GGS_string ("_stack")) COMMA_SOURCE_FILE_AT_LINE (22)) ;
  ::routine_doReplace (_inLexique,  var_cas_result,  GGS_string ("$EXEC_ALL_CONTEXT$"),  (var_cas_name.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (24)))._operator_concat (GGS_string ("_int_context")) COMMA_SOURCE_FILE_AT_LINE (24)) ;
  GGS_uint  var_cas_stack_size ;
  ::routine_additional_int_key_required (_inLexique,  GGS_string ("STACKSIZE"),  var_cas_others,  var_cas_name,  var_cas_stack_size COMMA_SOURCE_FILE_AT_LINE (27)) ;
  ::routine_doReplace (_inLexique,  var_cas_result,  GGS_string ("$STACK_SIZE$"),  var_cas_stack_size.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (28)) COMMA_SOURCE_FILE_AT_LINE (28)) ;
  { const GGS_oil_obj _var_1320 = var_cas_exe ; // CAST instruction
    if (_var_1320.getPtr () != NULL) {
      ...
      if (typeid (cPtr_isr_obj) == typeid (* (_var_1320.getPtr ()))) {
        ...
        ::routine_additional_string_key_required (_inLexique,  GGS_string ("SOURCE"),  var_cas_others,  var_cas_name,  var_cas_sig COMMA_SOURCE_FILE_AT_LINE (35)) ;
      }else{
      }
    }
  }
        ...
        { const GGS_basic_type _var_2167 = var_cas_value ; // CAST instruction
          if (_var_2167.getPtr () != NULL) {
            ...
            if (typeid (cPtr_string_class) == typeid (* (_var_2167.getPtr ()))) {
              ...
              var_cas_result = ((var_cas_result)._operator_concat (GGS_string ("\t")))._operator_concat (var_cas_s.reader_value (_inLexique COMMA_SOURCE_FILE_AT_LINE (66))) ;
            }else{
              operand_1642->key.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, (var_cas_key)._operator_concat (GGS_string (" should be a string")) COMMA_SOURCE_FILE_AT_LINE (69)) ;
            }
          }
        }
      }else{
        operand_1642->key.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, ((GGS_string ("no "))._operator_concat (var_cas_key))._operator_concat (GGS_string (" specified")) COMMA_SOURCE_FILE_AT_LINE (72)) ;
      }
    }
  }
  var_cas_code = var_cas_code.reader_stringByReplacingStringByString (_inLexique, GGS_string ("$ISR_SIGNAL_LIST$"), var_cas_result COMMA_SOURCE_FILE_AT_LINE (74)) ;
  #ifdef DEBUG_TRACE_ENABLED
...
void routine_generate_counter_libpcl (C_Compiler & _inLexique,
      ...
      { const GGS_counter_type _var_2777 = operand_2477->mInfo.counter.reader_type (_inLexique COMMA_SOURCE_FILE_AT_LINE (83)) ; // CAST instruction
        if (_var_2777.getPtr () != NULL) {
          ...
          if (typeid (cPtr_software_counter) == typeid (* (_var_2777.getPtr ()))) {
            ...
            ::routine_retrieveTemplateString (_inLexique,  var_cas_tmp,  GGS_string ("counter_list_specific") COMMA_SOURCE_FILE_AT_LINE (87)) ;
            ::routine_doReplace (_inLexique,  var_cas_tmp,  GGS_string ("$COUNTER$"),  (operand_2477->mKey.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (88)))._operator_concat (GGS_string ("_counter_desc")) COMMA_SOURCE_FILE_AT_LINE (88)) ;
  ...
  ::routine_doReplace (_inLexique,  var_cas_code,  GGS_string ("$COUNTER_LIST$"),  var_cas_result COMMA_SOURCE_FILE_AT_LINE (93)) ;
  #ifdef DEBUG_TRACE_ENABLED
