...
void routine_goilTemplateInvocation (C_Compiler & inLexique,
  ...
  #endif
  const GGS_bool cond_8825 = (GGS_uint ::constructor_errorCount (inLexique COMMA_HERE)).operator_isEqual (GGS_uint (0U)) ;
  if (cond_8825.isBuiltAndTrue ()) {
    const GGS_string var_cas_templateDir = gOption_goil_5F_options_goil_5F_template_5F_dir.value () ;
    const GGS_bool cond_7009 = (var_cas_templateDir.reader_length (inLexique COMMA_SOURCE_FILE_AT_LINE (189))).operator_isEqual (GGS_uint (0U)) ;
    if (cond_7009.isBuiltAndTrue ()) {
      var_cas_inTemplateName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("the '--goil-templates' option is required for generating code") COMMA_SOURCE_FILE_AT_LINE (191)) ;
    }else if (cond_7009.isBuiltAndFalse ()) {
      const GGS_bool cond_7184 = (var_cas_templateDir.reader_directoryExists (inLexique COMMA_SOURCE_FILE_AT_LINE (191))).operator_not () ;
      if (cond_7184.isBuiltAndTrue ()) {
        var_cas_inTemplateName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, ((GGS_string ("the '--goil-templates' option value is '")).operator_concat (var_cas_templateDir)).operator_concat (GGS_string ("', but is not a valid directory path")) COMMA_SOURCE_FILE_AT_LINE (193)) ;
      }else if (cond_7184.isBuiltAndFalse ()) {
        const GGS_string var_cas_targetDir = gOption_goil_5F_options_target_5F_platform.value () ;
        const GGS_bool cond_7404 = (var_cas_targetDir.reader_length (inLexique COMMA_SOURCE_FILE_AT_LINE (195))).operator_isEqual (GGS_uint (0U)) ;
        if (cond_7404.isBuiltAndTrue ()) {
          var_cas_inTemplateName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, GGS_string ("the '--target' (or '-t') option is required for generating code") COMMA_SOURCE_FILE_AT_LINE (197)) ;
        }else if (cond_7404.isBuiltAndFalse ()) {
          const GGS_bool cond_7724 = (((var_cas_templateDir).operator_concat (GGS_string ("/"))).operator_concat (var_cas_targetDir).reader_directoryExists (inLexique COMMA_SOURCE_FILE_AT_LINE (197))).operator_not () ;
          if (cond_7724.isBuiltAndTrue ()) {
          ...
          }else if (cond_7724.isBuiltAndFalse ()) {
            ...
            GGS_uint _variant_8497 = var_cas_targetDir.reader_length (inLexique COMMA_SOURCE_FILE_AT_LINE (206)) ;
            GGS_bool _condition_8497 (_variant_8497.isBuilt (), true) ;
            while (_condition_8497.isBuiltAndTrue ()) {
              _condition_8497 = ((var_cas_targetTemplatePartialPath.reader_length (inLexique COMMA_SOURCE_FILE_AT_LINE (207))).operator_strictSup (GGS_uint (0U))).operator_and ((var_cas_found).operator_not ()) ;
              if (_condition_8497.isBuiltAndTrue ()) {
                if (_variant_8497.uintValue () == 0) {
                  inLexique.onTheFlyRunTimeError ("loop variant error" COMMA_SOURCE_FILE_AT_LINE (217)) ;
                  _condition_8497 = GGS_bool (false) ;
                }else{
                  _variant_8497.decrement_operation (inLexique COMMA_HERE) ;
                  ...
                  var_cas_found = var_cas_fullTemplatePath.reader_fileExists (inLexique COMMA_SOURCE_FILE_AT_LINE (211)) ;
                  const GGS_bool cond_8361 = var_cas_found ;
                  if (cond_8361.isBuiltAndTrue ()) {
                  ...
                  }else if (cond_8361.isBuiltAndFalse ()) {
                    var_cas_targetTemplatePartialPath = var_cas_targetTemplatePartialPath.reader_stringByDeletingLastPathComponent (inLexique COMMA_SOURCE_FILE_AT_LINE (215)) ;
                  }
                }
              }
            }
            const GGS_bool cond_8786 = (var_cas_found).operator_not () ;
            if (cond_8786.isBuiltAndTrue ()) {
              ...
              {
                GGS_stringlist::cEnumerator enumerator_8666 (var_cas_pathList, true) ;
                const GGS_stringlist::cElement * operand_8666 = NULL ;
                while (((operand_8666 = enumerator_8666.nextObject ()))) {
                  macroValidPointer (operand_8666) ;
                  var_cas_m.appendCString ("\n"
                    "  - '") ;
                  var_cas_m.dotAssign_operation (operand_8666->mValue) ;
                  var_cas_m.appendCString ("';") ;
                }
              }
              var_cas_inTemplateName.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (224)) ;
            }
          }
        }
      }
