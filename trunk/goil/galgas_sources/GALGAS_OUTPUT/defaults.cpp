...
void routine_check_custom_field (C_Compiler & inLexique,
  ...
  #endif
  const GGS_bool cond_1291 = (var_cas_others.reader_hasKey (var_cas_field.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (39)))).operator_not () ;
...
void routine_add_system_counter (C_Compiler & inLexique,
  ...
  const GGS_bool cond_5824 = var_cas_counters.reader_hasKey (GGS_string ("SystemCounter")) ;
  if (cond_5824.isBuiltAndTrue ()) {
    ...
    const GGS_bool cond_5813 = gOption_goil_5F_options_autosar_5F_on.reader_value () ;
    if (cond_5813.isBuiltAndTrue ()) {
      const GGS_bool cond_5634 = (var_cas_sys_cnt.reader_acc_apps (inLexique COMMA_SOURCE_FILE_AT_LINE (193)).reader_length ()).operator_isEqual (GGS_uint (0U)) ;
      if (cond_5634.isBuiltAndTrue ()) {
        var_cas_sys_cnt.modifier_setAcc_apps (inLexique, var_cas_all_apps COMMA_SOURCE_FILE_AT_LINE (194)) ;
      }
      { const GGS_counter_type _var_5799 = var_cas_sys_cnt.reader_type (inLexique COMMA_SOURCE_FILE_AT_LINE (196)) ; // CAST instruction
        if (_var_5799.getPtr () != NULL) {
          macroValidPointer (_var_5799.getPtr ()) ;
          if (typeid (cPtr_software_counter) == typeid (* (_var_5799.getPtr ()))) {
            const GGS_software_counter var_cas_sc (_var_5799.getPtr ()) ;
  ...
  }else if (cond_5824.isBuiltAndFalse ()) {
...
void routine_set_defaults (C_Compiler & inLexique,
  ...
  #endif
  const GGS_bool cond_6946 = var_cas_imp.reader_hasKey (GGS_string ("task")) ;
  if (cond_6946.isBuiltAndTrue ()) {
    ...
    const GGS_implementation  temp_6792 = var_cas_imp ;
    if (temp_6792.isBuilt ()) {
      temp_6792 (HERE)->method_get (inLexique, GGS_lstring ::constructor_new (inLexique, GGS_string ("task"), GGS_location (inLexique) COMMA_HERE), var_cas_spec COMMA_SOURCE_FILE_AT_LINE (229)) ;
    }
  ...
  const GGS_bool cond_7207 = var_cas_imp.reader_hasKey (GGS_string ("isr")) ;
  if (cond_7207.isBuiltAndTrue ()) {
    GGS_isr_map  var_cas_isrs = var_cas_cpu.reader_isrs (inLexique COMMA_SOURCE_FILE_AT_LINE (237)) ;
    GGS_implementation_spec  var_cas_spec ;
    const GGS_implementation  temp_7086 = var_cas_imp ;
    if (temp_7086.isBuilt ()) {
      temp_7086 (HERE)->method_get (inLexique, GGS_lstring ::constructor_new (inLexique, GGS_string ("isr"), GGS_location (inLexique) COMMA_HERE), var_cas_spec COMMA_SOURCE_FILE_AT_LINE (239)) ;
    }
