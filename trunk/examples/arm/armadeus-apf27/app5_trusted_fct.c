...
extern CONST(TrustedFunctionIndexType, AUTOMATIC) term_print;
/**
 * term_print trusted function wrapper
 */
FUNC(void, OS_APPL_CODE) app3_term_print (char *string)
{
  CallTrustedFunction (term_print, (TrustedFunctionParameterRefType)string);
}

FUNC(void, OS_APPL_CODE) TRUSTED_term_print (TrustedFunctionIndexType function, TrustedFunctionParameterRefType parameter)
{
  serial_puts ((char *)parameter);
}
...
TASK(t1)
{
  int i;
  TRACE_OSAP1(10);
  app3_term_print ("TASK t1 writes to large shared memory space\n");
  for (i = 0 ; i < (OSAP1_LSM_SIZE) ; i++)
  {
