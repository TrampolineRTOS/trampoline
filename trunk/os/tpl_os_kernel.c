...
/*
 * MISRA RULE 27 VIOLATION: These 5 variables are used only in this file
 ...
 * the application description. Indexes of this table are the priority levels
 * each of its elements is a read and write index in the fifo.
 */
 ...
 * the application description. Indexes of this table are the tasks' identifiers.
 ...
 * the application description. Indexes of this table are the alarms' identifiers.
 * each of its elements is an application mode mask. For an AUTOSTART alarm with
 * identifier id in application mode app_mode, tpl_alarm_app_mode[id] & app_mode
 * will be true.
 */
 ...
 * @internal
 *
 * tpl_scheduletable_app_mode is a table that is automatically generated by goil from
 * the application description. Indexes of this table are the schedule tables'
 * identifiers. each of its elements is an application mode mask. For an AUTOSTART 
 * schedule table with identifier id in application mode app_mode,
 * tpl_scheduletable_app_mode[id] & app_mode will be true.
 */
extern CONST(tpl_appmode_mask, OS_CONST) tpl_scheduletable_app_mode[SCHEDTABLE_COUNT];
  ...
  CONST(tpl_proc_id, AUTOMATIC) proc_id)
{
  ...
  /* A newly activated executable object is put at the end of the fifo */
...
FUNC(tpl_os_state, OS_CODE) tpl_current_os_state(void)
{
  ...
  if (tpl_kern.running_id == INVALID_PROC_ID) {
    state = OS_INIT;
  }
  else if (tpl_kern.running_id == IDLE_TASK_ID)
  {
    state = OS_IDLE;
  }
  ...
  else if (tpl_kern.running_id < (TASK_COUNT + ISR_COUNT) )
  {
    state = OS_ISR2;
  }
 ...
 */
FUNC(void, OS_CODE) tpl_preempt(void)
{
