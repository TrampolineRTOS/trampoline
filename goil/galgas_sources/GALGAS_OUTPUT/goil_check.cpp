...
void routine_check_uint32_range (C_Compiler & inLexique,
  ...
  #endif
  { const GGS_uint32_set _var_1243 = var_cas_set ; // CAST instruction
    if (_var_1243.getPtr () != NULL) {
      macroValidPointer (_var_1243.getPtr ()) ;
      if (typeid (cPtr_uint32_range) == typeid (* (_var_1243.getPtr ()))) {
        const GGS_uint32_range var_cas_r (_var_1243.getPtr ()) ;
        const GGS_bool cond_831 = ((var_cas_val).operator_strictInf (var_cas_r.reader_min (inLexique COMMA_SOURCE_FILE_AT_LINE (38)))).operator_or ((var_cas_val).operator_strictSup (var_cas_r.reader_max (inLexique COMMA_SOURCE_FILE_AT_LINE (38)))) ;
        if (cond_831.isBuiltAndTrue ()) {
      ...
      }else if (typeid (cPtr_uint32_list) == typeid (* (_var_1243.getPtr ()))) {
        const GGS_uint32_list var_cas_l (_var_1243.getPtr ()) ;
        GGS_bool var_cas_not_matched = GGS_bool (true) ;
        {
          GGS_list_uint32::cEnumerator enumerator_920 (var_cas_l.reader_li (inLexique COMMA_SOURCE_FILE_AT_LINE (44)), true) ;
          const GGS_list_uint32::cElement * operand_920 = NULL ;
          while (((operand_920 = enumerator_920.nextObject ()))) {
            macroValidPointer (operand_920) ;
            const GGS_bool cond_1036 = (var_cas_val).operator_isEqual (operand_920->val) ;
            if (cond_1036.isBuiltAndTrue ()) {
              var_cas_not_matched = GGS_bool (false) ;
            }
          }
        }
        const GGS_bool cond_1225 = var_cas_not_matched ;
        if (cond_1225.isBuiltAndTrue ()) {
