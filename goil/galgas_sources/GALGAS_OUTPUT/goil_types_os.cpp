...
method_generate_c (C_Compiler & _inLexique,
  ...
  var_cas_res.appendCstring ("#define    TRACE_FORMAT(a)        tpl_trace_format_") ;
  ...
  const GGS_trace_method  _temp_1719 = meth ;
  if (_temp_1719._isBuilt ()) {
    _temp_1719 (HERE)->method_generate_c (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (73)) ;
  }
...
void routine_set_method (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_trace_method _var_3035 = var_cas_t ; // CAST instruction
    if (_var_3035.getPtr () != NULL) {
      macroValidPointer (_var_3035.getPtr ()) ;
      if (typeid (cPtr_trace_void) == typeid (* (_var_3035.getPtr ()))) {
        var_cas_t = var_cas_s ;
      }else{
        var_cas_s.reader_loc (_inLexique COMMA_SOURCE_FILE_AT_LINE (124)).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, (GGS_string (true, "Redefinition of "))._operator_concat (var_cas_att) COMMA_SOURCE_FILE_AT_LINE (125)) ;
        var_cas_t.reader_loc (_inLexique COMMA_SOURCE_FILE_AT_LINE (125)).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "was defined here") COMMA_SOURCE_FILE_AT_LINE (126)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
