...
method_generate_c (C_Compiler & _inLexique,
  ...
  var_cas_res.appendCstring ("#define    TRACE_FORMAT   ") ;
  ...
  const GGS_trace_method  _temp_1658 = meth ;
  if (_temp_1658._isBuilt ()) {
    _temp_1658 (HERE)->method_generate_c (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (73)) ;
  }
...
void routine_set_method (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_trace_method _var_2974 = var_cas_t ; // CAST instruction
    if (_var_2974.getPtr () != NULL) {
      macroValidPointer (_var_2974.getPtr ()) ;
      if (typeid (cPtr_trace_void) == typeid (* (_var_2974.getPtr ()))) {
        var_cas_t = var_cas_s ;
      }else{
