...
method_generate_c (C_Compiler & _inLexique,
  ...
  var_cas_res.appendCstring ("#define    TRACE_FORMAT(p) tpl_trace_format_") ;
  var_cas_res._dotAssign_operation (form.reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (71))) ;
  var_cas_res.appendCstring ("(p);\n") ;
  GGS_string var_cas_m ;
  const GGS_trace_method  _temp_1684 = meth ;
  if (_temp_1684._isBuilt ()) {
    _temp_1684 (HERE)->method_generate_c (_inLexique, var_cas_m COMMA_SOURCE_FILE_AT_LINE (73)) ;
  }
...
void routine_set_method (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_trace_method _var_3000 = var_cas_t ; // CAST instruction
    if (_var_3000.getPtr () != NULL) {
      macroValidPointer (_var_3000.getPtr ()) ;
      if (typeid (cPtr_trace_void) == typeid (* (_var_3000.getPtr ()))) {
        var_cas_t = var_cas_s ;
      }else{
