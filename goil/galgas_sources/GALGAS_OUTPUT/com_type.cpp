...
GALGAS_com function_void_5F_com_5F_obj (C_Compiler * inCompiler
  ...
  //-------- @localVariableOrConstantDeclarationWithSourceExpressionForGeneration
  GALGAS_lstring var_emptyString = GALGAS_lstring::constructor_new (GALGAS_string::makeEmptyString (), inCompiler->here () COMMA_SOURCE_FILE_AT_LINE (83)) ;
...
typeComparisonResult cPtr_com::dynamicObjectCompare (const acPtr_class * inOperandPtr) const {
  typeComparisonResult result = kOperandEqual ;
  const cPtr_com * p = (const cPtr_com *) inOperandPtr ;
  macroValidSharedObject (p, const cPtr_com) ;
  if (kOperandEqual == result) {
    result = mAttribute_desc.objectCompare (p->mAttribute_desc) ;
  }
  if (kOperandEqual == result) {
    result = mAttribute_usecom.objectCompare (p->mAttribute_usecom) ;
  }
  if (kOperandEqual == result) {
    result = mAttribute_timebase.objectCompare (p->mAttribute_timebase) ;
  }
  if (kOperandEqual == result) {
  ...
  if (kOperandEqual == result) {
    result = mAttribute_startcomextension.objectCompare (p->mAttribute_startcomextension) ;
  }
  if (kOperandEqual == result) {
  ...
  if (kOperandEqual == result) {
    result = mAttribute_uses.objectCompare (p->mAttribute_uses) ;
  }
  return result ;
}
...
typeComparisonResult GALGAS_com::objectCompare (const GALGAS_com & inOperand) const {
