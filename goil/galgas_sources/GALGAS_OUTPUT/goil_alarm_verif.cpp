...
void routine_counters_of_alarms_defined (C_Compiler & inLexique,
  ...
  #endif
  {
    GGS_alarm_map::cEnumerator enumerator_338 (var_cas_alarms, true) ;
    const GGS_alarm_map::cElement * operand_338 = NULL ;
    while (((operand_338 = enumerator_338.nextObject ()))) {
      macroValidPointer (operand_338) ;
      GGS_lstring  var_cas_counter = operand_338->mInfo.alarm.reader_counter_name (inLexique COMMA_SOURCE_FILE_AT_LINE (16)) ;
      const GGS_bool cond_647 = ((var_cas_counters.reader_hasKey (var_cas_counter.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (17)))).operator_not ()).operator_and ((var_cas_counter.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (17))).operator_isNotEqual (GGS_string ("SystemCounter"))) ;
      if (cond_647.isBuiltAndTrue ()) {
      ...
      }else if (cond_647.isBuiltAndFalse ()) {
        { const GGS_autostart_obj _var_1860 = operand_338->mInfo.alarm.reader_autostart_params (inLexique COMMA_SOURCE_FILE_AT_LINE (25)) ; // CAST instruction
          if (_var_1860.getPtr () != NULL) {
            macroValidPointer (_var_1860.getPtr ()) ;
            if (typeid (cPtr_alarm_autostart_true) == typeid (* (_var_1860.getPtr ()))) {
              const GGS_alarm_autostart_true var_cas_a (_var_1860.getPtr ()) ;
              ...
              const GGS_bool cond_1374 = (var_cas_alarm_time).operator_isEqual (GGS_uint (0U)) ;
              if (cond_1374.isBuiltAndTrue ()) {
                const GGS_bool cond_1255 = (gOption_goil_5F_options_autosar_5F_on.reader_value ()).operator_isEqual (GGS_bool (true)) ;
                if (cond_1255.isBuiltAndTrue ()) {
                ...
                }else if (cond_1255.isBuiltAndFalse ()) {
              ...
              const GGS_counter_map  temp_1435 = var_cas_counters ;
              if (temp_1435.isBuilt ()) {
                temp_1435 (HERE)->method_getCounter (inLexique, var_cas_counter, var_cas_ct COMMA_SOURCE_FILE_AT_LINE (41)) ;
              }
              { const GGS_basic_type _var_1838 = var_cas_ct.reader_max_allowed_value (inLexique COMMA_SOURCE_FILE_AT_LINE (42)) ; // CAST instruction
                if (_var_1838.getPtr () != NULL) {
                  macroValidPointer (_var_1838.getPtr ()) ;
                  if (typeid (cPtr_uint32_class) == typeid (* (_var_1838.getPtr ()))) {
                    const GGS_uint32_class var_cas_mc (_var_1838.getPtr ()) ;
                    const GGS_bool cond_1668 = (var_cas_alarm_time).operator_strictSup (var_cas_mc.reader_value (inLexique COMMA_SOURCE_FILE_AT_LINE (44))) ;
                    if (cond_1668.isBuiltAndTrue ()) {
                    ...
                    const GGS_bool cond_1816 = (var_cas_cycle_time).operator_strictSup (var_cas_mc.reader_value (inLexique COMMA_SOURCE_FILE_AT_LINE (47))) ;
