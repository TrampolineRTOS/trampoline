...
void routine_bool_flag (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_basic_type _var_7977 = var_cas_b ; // CAST instruction
    if (_var_7977.getPtr () != NULL) {
      macroValidPointer (_var_7977.getPtr ()) ;
      if (typeid (cPtr_void) == typeid (* (_var_7977.getPtr ()))) {
      }else if (typeid (cPtr_bool_class) == typeid (* (_var_7977.getPtr ()))) {
        const GGS_bool_class var_cas_bo (_var_7977.getPtr ()) ;
        if ((var_cas_bo.reader_value (_inLexique COMMA_SOURCE_FILE_AT_LINE (319))).isBuiltAndTrue ()) {
          var_cas_res.appendCstring ("#define ") ;
          var_cas_res._dotAssign_operation (var_cas_flag) ;
          var_cas_res.appendCstring ("\n") ;
        }
      }else{
        var_cas_b.reader_location (_inLexique COMMA_SOURCE_FILE_AT_LINE (325)).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "Internal error, wrong datatype") COMMA_SOURCE_FILE_AT_LINE (326)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
