...
//---------------------------------------------------------------------------*
//                                                                           *
//                   Implementation of routine "get_bool"                    *
//                                                                           *
//---------------------------------------------------------------------------*

void routine_get_bool (C_Compiler & _inLexique,
                                GGS_basic_type   var_cas_t,
                                GGS_bool & var_cas_b COMMA_UNUSED_LOCATION_ARGS) {
  #ifdef DEBUG_TRACE_ENABLED
    printf ("ENTER routine_get_bool at %s:%d\n", __FILE__, __LINE__) ;
  #endif
  { const GGS_basic_type _var_7741 = var_cas_t ; // CAST instruction
    if (_var_7741.getPtr () != NULL) {
      macroValidPointer (_var_7741.getPtr ()) ;
      if (typeid (cPtr_bool_class) == typeid (* (_var_7741.getPtr ()))) {
        const GGS_bool_class var_cas_bc (_var_7741.getPtr ()) ;
        var_cas_b = var_cas_bc.reader_value (_inLexique COMMA_SOURCE_FILE_AT_LINE (316)) ;
      }else{
        var_cas_b = GGS_bool (true, false) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
    printf ("LEAVE routine_get_bool\n") ;
  #endif
}
...
void routine_bool_flag (C_Compiler & _inLexique,
  ...
  #endif
  { const GGS_basic_type _var_8143 = var_cas_b ; // CAST instruction
    if (_var_8143.getPtr () != NULL) {
      macroValidPointer (_var_8143.getPtr ()) ;
      if (typeid (cPtr_void) == typeid (* (_var_8143.getPtr ()))) {
      }else if (typeid (cPtr_bool_class) == typeid (* (_var_8143.getPtr ()))) {
        const GGS_bool_class var_cas_bo (_var_8143.getPtr ()) ;
        if ((var_cas_bo.reader_value (_inLexique COMMA_SOURCE_FILE_AT_LINE (331))).isBuiltAndTrue ()) {
          var_cas_res.appendCstring ("#define ") ;
          var_cas_res._dotAssign_operation (var_cas_flag) ;
          var_cas_res.appendCstring ("\n") ;
        }
      }else{
        var_cas_b.reader_location (_inLexique COMMA_SOURCE_FILE_AT_LINE (337)).reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, GGS_string (true, "Internal error, wrong datatype") COMMA_SOURCE_FILE_AT_LINE (338)) ;
      }
    }
  }
  #ifdef DEBUG_TRACE_ENABLED
