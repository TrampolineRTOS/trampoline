...
void routine_generate_counter_ppc (C_Compiler & inLexique,
  ...
  GGS_string var_cas_result = GGS_string ("") ;
  {
    ...
    while (((operand_2973 = enumerator_2973.nextObject ()))) {
      ...
      { const GGS_counter_type _var_3265 = operand_2973->mInfo.counter.reader_type (inLexique COMMA_SOURCE_FILE_AT_LINE (93)) ; // CAST instruction
        if (_var_3265.getPtr () != NULL) {
          macroValidPointer (_var_3265.getPtr ()) ;
          if (typeid (cPtr_software_counter) == typeid (* (_var_3265.getPtr ()))) {
          }else{
            GGS_string var_cas_tmp = function_template_string (inLexique, GGS_string ("code"), GGS_string ("counter_list_specific") COMMA_SOURCE_FILE_AT_LINE (96)) ;
            ::routine_doReplace (inLexique,  var_cas_tmp,  GGS_string ("$COUNTER$"),  (operand_2973->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (97))).operator_concat (GGS_string ("_counter_desc")) COMMA_SOURCE_FILE_AT_LINE (97)) ;
  ...
  ::routine_doReplace (inLexique,  var_cas_code,  GGS_string ("$COUNTER_LIST$"),  var_cas_result COMMA_SOURCE_FILE_AT_LINE (102)) ;
  #ifdef DEBUG_TRACE_ENABLED
