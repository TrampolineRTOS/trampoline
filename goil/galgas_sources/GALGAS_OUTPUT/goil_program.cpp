...
//--- Fix parameters for BDD package
  ...
  //--- Input/output parameters
    ...
    const char * extensions [] = {"oil", "OIL", "epc", NULL} ;
    const char * helpMessages [] = {"an '.oil' source file", "an '.OIL' source file", "a XML source file", NULL} ;
    ...
    try{
      ...
      for (sint32 i=0 ; i<sourceFilesArray.count () ; i++) {
        try {
          ...
          if (fileExtension.compare ("oil") == 0) {
          ...
          }else if (fileExtension.compare ("epc") == 0) {
            C_Compiler & _inLexique = * _commonLexique ;
            const GGS_string _source (true, sourceFilesArray (i COMMA_HERE)) ;
            const GGS_location _here (_inLexique) ;
            const GGS_lstring var_cas_inSourceFile (GGS_lstring::constructor_new (_inLexique, _source, _here COMMA_HERE)) ;
            GGS_xmlNodeList  var_cas_root_nodes ;
            const GGS_string _depExtension = GGS_string ("") ;
            const GGS_string _depPath = GGS_string ("") ;
            if (_depExtension.isBuilt () && _depPath.isBuilt ()) {
            ...
            ::routine_logXMLNodes (_inLexique,  var_cas_root_nodes COMMA_SOURCE_FILE_AT_LINE (27)) ;
            GGS_root_obj  var_cas_cpu ;
            var_cas_cpu = function_transform_xml_to_goil (_inLexique, var_cas_root_nodes COMMA_SOURCE_FILE_AT_LINE (29)) ;
            if (verboseOptionOn) {
              co << "Analysis of '" << sourceFilesArray (i COMMA_HERE).lastPathComponent () << "' completed. " ;
              switch (_commonLexique->totalErrorCount ()) {
              case 0 :
