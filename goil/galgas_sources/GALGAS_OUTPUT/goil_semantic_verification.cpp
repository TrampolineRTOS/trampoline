...
void routine_all_alarm_actions_correct (C_Compiler & _inLexique,
  ...
  var_cas_defined_task_names = var_cas_tasks.reader_allKeys (_inLexique COMMA_SOURCE_FILE_AT_LINE (373)) ;
  {
    ...
    while (((operand_16098 = enumerator_16098.nextObject ()))) {
      ...
      { const GGS_action_obj _var_18015 = var_cas_alarm_action ; // CAST instruction
        if (_var_18015.getPtr () != NULL) {
          macroValidPointer (_var_18015.getPtr ()) ;
          if (typeid (cPtr_setevent_action) == typeid (* (_var_18015.getPtr ()))) {
            const GGS_setevent_action var_cas_act (_var_18015.getPtr ()) ;
          ...
          }else if (typeid (cPtr_incrementcounter_action) == typeid (* (_var_18015.getPtr ()))) {
            const GGS_incrementcounter_action var_cas_act (_var_18015.getPtr ()) ;
            if ((var_cas_counters.reader_hasKey (_inLexique, var_cas_act.reader_counter_name (_inLexique COMMA_SOURCE_FILE_AT_LINE (397)).reader_string (_inLexique COMMA_SOURCE_FILE_AT_LINE (397)) COMMA_SOURCE_FILE_AT_LINE (397))).isBuiltAndTrue ()) {
              ...
              const GGS_counter_map  _temp_17520 = var_cas_counters ;
              if (_temp_17520._isBuilt ()) {
                _temp_17520 (HERE)->method_get (_inLexique, var_cas_act.reader_counter_name (_inLexique COMMA_SOURCE_FILE_AT_LINE (399)), var_cas_cnt COMMA_SOURCE_FILE_AT_LINE (399)) ;
              }
              { const GGS_counter_type _var_17809 = var_cas_cnt.reader_type (_inLexique COMMA_SOURCE_FILE_AT_LINE (400)) ; // CAST instruction
                if (_var_17809.getPtr () != NULL) {
                  macroValidPointer (_var_17809.getPtr ()) ;
                  if (typeid (cPtr_software_counter) == typeid (* (_var_17809.getPtr ()))) {
                  }else{
