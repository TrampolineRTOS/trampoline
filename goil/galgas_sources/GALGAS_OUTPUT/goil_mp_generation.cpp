...
void routine_generate_mp (C_Compiler & inLexique,
  ...
  var_cas_stasks = GGS_tasks_by_type ::constructor_emptySortedList () ;
  {
    ...
    while (((operand_4793 = enumerator_4793.nextObject ()))) {
      ...
      if (((operand_4793->mInfo.task.reader_events (inLexique COMMA_SOURCE_FILE_AT_LINE (123)).reader_length (inLexique COMMA_SOURCE_FILE_AT_LINE (123))).operator_strictSup (GGS_uint (0U))).isBuiltAndTrue ()) {
        var_cas_stasks._addAssign_operation (GGS_uint (0U), operand_4793->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (124))) ;
      }else{
        var_cas_stasks._addAssign_operation (GGS_uint (1U), operand_4793->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (126))) ;
      }
    }
  }
