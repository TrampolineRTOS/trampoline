  ...
  if (((GGS_bool (inLexique.boolOptionValueFromKeys ("goil_options", "posix_it" COMMA_SOURCE_FILE_AT_LINE (41)))).operator_not ()).isBuiltAndTrue ()) {
    {
      ...
      while (((operand_1238 = enumerator_1238.nextObject ()))) {
        ...
        var_cas_list_isr._addAssign_operation (operand_1238->mKey, operand_1238->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (43))) ;
      }
    }
    {
      ...
      while (((operand_1356 = enumerator_1356.nextObject ()))) {
        ...
        const GGS_isr_map  temp_1417 = var_cas_isrs ;
        if (temp_1417.isBuilt ()) {
          temp_1417 (HERE)->method_get (inLexique, operand_1356->key, var_cas_isr COMMA_SOURCE_FILE_AT_LINE (48)) ;
        }
        ...
        if ((var_cas_others.reader_hasKey (inLexique, var_cas_key COMMA_SOURCE_FILE_AT_LINE (54))).isBuiltAndTrue ()) {
          ...
          const GGS_ident_map  temp_1682 = var_cas_others ;
          if (temp_1682.isBuilt ()) {
            temp_1682 (HERE)->method_get (inLexique, GGS_lstring ::constructor_new (inLexique, var_cas_key, GGS_location (inLexique) COMMA_HERE), var_cas_value COMMA_SOURCE_FILE_AT_LINE (57)) ;
          }
          { const GGS_basic_type _var_1915 = var_cas_value ; // CAST instruction
            if (_var_1915.getPtr () != NULL) {
              ...
              if (typeid (cPtr_string_class) == typeid (* (_var_1915.getPtr ()))) {
                ...
                operand_1356->key.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, (var_cas_key).operator_concat (GGS_string (" should be a string")) COMMA_SOURCE_FILE_AT_LINE (63)) ;
              }
            }
          }
        }else{
          operand_1356->key.reader_location (inLexique COMMA_HERE).signalGGSSemanticError (inLexique, ((GGS_string ("no ")).operator_concat (var_cas_key)).operator_concat (GGS_string (" specified")) COMMA_SOURCE_FILE_AT_LINE (66)) ;
        }
      }
    }
  }
...
void routine_generate_counter_posix (C_Compiler & inLexique,
  ...
  var_cas_result = GGS_string ("") ;
  {
    ...
    while (((operand_2242 = enumerator_2242.nextObject ()))) {
      ...
      { const GGS_counter_type _var_2542 = operand_2242->mInfo.counter.reader_type (inLexique COMMA_SOURCE_FILE_AT_LINE (78)) ; // CAST instruction
        if (_var_2542.getPtr () != NULL) {
          ...
          if (typeid (cPtr_software_counter) == typeid (* (_var_2542.getPtr ()))) {
            ...
            ::routine_doReplace (inLexique,  var_cas_tmp,  GGS_string ("$COUNTER$"),  (operand_2242->mKey.reader_string (inLexique COMMA_SOURCE_FILE_AT_LINE (83))).operator_concat (GGS_string ("_counter_desc")) COMMA_SOURCE_FILE_AT_LINE (83)) ;
