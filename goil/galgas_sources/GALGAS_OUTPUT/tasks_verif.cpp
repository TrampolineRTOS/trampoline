...
void routine_tasks_5F_well_5F_formed (const GALGAS_task_5F_map constinArgument_tasks,
  ...
  cEnumerator_task_5F_map enumerator_841 (constinArgument_tasks, true) ;
  ...
  while (enumerator_841.hasCurrentObject ()) {
    ...
    if (enumerator_841.current_task (HERE).reader_activation (SOURCE_FILE_AT_LINE (54)).isValid ()) {
      if (enumerator_841.current_task (HERE).reader_activation (SOURCE_FILE_AT_LINE (54)).dynamicTypeDescriptor () == & kTypeDescriptor_GALGAS_void) {
        ...
        if (GALGAS_bool (kIsStrictSup, var_defined_5F_events.reader_length (SOURCE_FILE_AT_LINE (59)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).operator_and (GALGAS_bool (kIsStrictSup, cast_1214_ui.reader_value (SOURCE_FILE_AT_LINE (59)).objectCompare (GALGAS_uint ((PMUInt32) 1U))) COMMA_SOURCE_FILE_AT_LINE (59)).isValidAndTrue ()) {
          //-------- @errorInstructionForGeneration
          GALGAS_location location_2 (enumerator_841.current_lkey (HERE).reader_location (HERE)) ; // Implicit use of 'location' reader
          inCompiler->emitSemanticError (location_2, GALGAS_string ("ACTIVATION attribute for extended task ").operator_concat (enumerator_841.current_lkey (HERE).reader_string (SOURCE_FILE_AT_LINE (60)) COMMA_SOURCE_FILE_AT_LINE (60)).operator_concat (GALGAS_string (" should be set to 1") COMMA_SOURCE_FILE_AT_LINE (60)) COMMA_SOURCE_FILE_AT_LINE (60)) ;
          //-------- @errorInstructionForGeneration
    ...
    //-------- @ifInstructionForGeneration
    if (GALGAS_bool (kIsEqual, enumerator_841.current_task (HERE).reader_schedule (SOURCE_FILE_AT_LINE (67)).reader_string (SOURCE_FILE_AT_LINE (67)).objectCompare (GALGAS_string (""))).isValidAndTrue ()) {
      //-------- @errorInstructionForGeneration
      GALGAS_location location_3 (enumerator_841.current_lkey (HERE).reader_location (HERE)) ; // Implicit use of 'location' reader
      inCompiler->emitSemanticError (location_3, GALGAS_string ("SCHEDULE attribute not defined for TASK ").operator_concat (enumerator_841.current_lkey (HERE).reader_string (SOURCE_FILE_AT_LINE (68)) COMMA_SOURCE_FILE_AT_LINE (68)) COMMA_SOURCE_FILE_AT_LINE (68)) ;
    }
    //-------- @structuredCastInstructionForGeneration
    if (enumerator_841.current_task (HERE).reader_autostart (SOURCE_FILE_AT_LINE (71)).isValid ()) {
      if (enumerator_841.current_task (HERE).reader_autostart (SOURCE_FILE_AT_LINE (71)).dynamicTypeDescriptor () == & kTypeDescriptor_GALGAS_task_5F_autostart_5F_void) {
        //-------- @errorInstructionForGeneration
        GALGAS_location location_4 (enumerator_841.current_lkey (HERE).reader_location (HERE)) ; // Implicit use of 'location' reader
        inCompiler->emitSemanticError (location_4, GALGAS_string ("AUTOSTART attribute not defined for TASK ").operator_concat (enumerator_841.current_lkey (HERE).reader_string (SOURCE_FILE_AT_LINE (73)) COMMA_SOURCE_FILE_AT_LINE (73)) COMMA_SOURCE_FILE_AT_LINE (73)) ;
      }
    }
