...
appendForMapDescription (C_Compiler & _inLexique,
  ...
  ioString << "|-value " ;
  ioString.appendSigned (inElementIndex) ;
  ioString << ":" << mInfo.tpl_attr.reader_description  (_inLexique COMMA_THERE, inIndentation + 1) ;
}
...
isEqualToObject (const cPtr__AC_galgas_class * inOperand) const {
  bool equal = typeid (this) == typeid (inOperand) ;
  if (equal) {
    const cPtr_interrupts * _p = dynamic_cast <const cPtr_interrupts *> (inOperand) ;
    macroValidPointer (_p) ;
    equal = location._operator_isEqual (_p->location).boolValue ()
         && it_sources._operator_isEqual (_p->it_sources).boolValue () ;
  }
  return equal ;
}
...
appendForDescription (C_Compiler & _inLexique,
  ...
  ioString << "->@interrupts:"
           << location.reader_description  (_inLexique COMMA_THERE, inIndentation + 1)
           << it_sources.reader_description  (_inLexique COMMA_THERE, inIndentation + 1) ;
}
...
cPtr__AC_galgas_class * cPtr_interrupts::makeClone (void) const {
  cPtr__AC_galgas_class * result = NULL ;
  macroMyNew (result, cPtr_interrupts (location, it_sources COMMA_HERE)) ;
  return result ;
}
...
GGS_interrupts::
GGS_interrupts (cPtr__AC_galgas_class * inPointer) {
  macroAssignPointer (mPointer, inPointer) ;
}
...
_castFrom (C_Compiler & inLexique,
  ...
  if (inPointer != NULL) {
    ...
    if (ok) {
      _result = GGS_interrupts (inPointer) ;
    }else{
      inErrorLocation.signalCastError (inLexique,
