	    ...
	    size_t size, int endian, size_t nail, mpz_srcptr z)
{
  ...
  size_t         count, dummy;
  unsigned long  numbx;
  size_t  align; // unsigned -> size_t by PM (nov 7, 2023)
...
//  align = (unsigned) ((char *) data - (char *) NULL) % sizeof (mp_limb_t); // (unsigned) added by PM
  align = (size_t) ((char *) data) % sizeof (mp_limb_t) ; // (char *) NULL removed by by PM (may 11, 2022)
...

  {
    ...
    woffset = (endian >= 0 ? ((mp_size_t) size) : - ((mp_size_t) size))
      + (order < 0 ? ((mp_size_t) size) : - (mp_size_t) size); // (mp_size_t) added by PM

    /* least significant byte */
    dp = (unsigned char *) data
      + (order >= 0 ? (count-1)*size : 0) + (endian >= 0 ? size-1 : 0);

// Two (unsigned char) added by PM
#define EXTRACT(N, MASK)                                \
	...
	for ( ; j < (mp_size_t) size; j++) // (mp_size_t) added by PM
	  {
	    *dp = '\0';
	    dp -= endian;
	  }
