	 ...
	 mp_srcptr vp, mp_size_t vn)
{
  ...
  if (un == vn)
      ...
      if (un >= 3 * vn)
	{
	  ...
	  ws = TMP_SALLOC_LIMBS (4 * (size_t) vn); // (size_t) added by PM
	  ...
	  if (2 * un >= 5 * vn)
	    {
	      ...
	      /* The maximum ws usage is for the mpn_mul result.  */
	      ws = TMP_SALLOC_LIMBS (7 * ((size_t) (vn >> 1))); // (size_t) added by PM
      ...
      if (un >= 8 * vn)
	{
	  ...
	  /* The maximum ws usage is for the mpn_mul result.  */
	  ws = TMP_BALLOC_LIMBS (9 * ((size_t) (vn >> 1))); // (size_t) added by PM
