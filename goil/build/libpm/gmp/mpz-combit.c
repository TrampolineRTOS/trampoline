...
mpz_combit (mpz_ptr d, mp_bitcnt_t bit_index)
{
  ...
  mp_size_t limb_index = (mp_size_t) (bit_index / GMP_NUMB_BITS);
  ...
  /* Check for the hairy case. d < 0, and we have all zero bits to the
      ...
      if (dp[limb_index] & bit)
	{
	  ...
	  SIZ(d) = - ((int) dsize) - ((int) dp[dsize]); // (int) added by PM
	}
      else
	{
	  ...
	  if (UNLIKELY ((dlimb == 0) + limb_index == dsize)) /* dsize == limb_index + 1 */
	    {
	      ...
	      SIZ (d) = SIZ (d) >= 0 ? ((int) limb_index) : -(int) limb_index; // (int)  added by PM
	    }
	}
      else
	{
	  ...
	  dp[limb_index++] = bit;
	  SIZ(d) = SIZ(d) >= 0 ? ((int) limb_index) : - (int) limb_index; // Added by PM
	}
    }
}
