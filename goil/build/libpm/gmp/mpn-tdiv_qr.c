	     ...
	     mp_srcptr np, mp_size_t nn, mp_srcptr dp, mp_size_t dn)
{
  ...
  switch (dn)
	...
	if ((dp[1] & GMP_NUMB_HIGHBIT) == 0)
	  {
	    unsigned long cnt; // int changed to unsigned long by PM
	    mp_limb_t dtmp[2];
	    ...
	    cy = mpn_lshift (n2p, np, nn, (unsigned int) cnt); // (unsigned int) added by PM
	    n2p[nn] = cy;
	...
	if (nn + adjust >= 2 * dn)
	  {
		...
		mpn_lshift (d2p, dp, dn, (unsigned int) cnt); // (unsigned int)  added by PM
		n2p = TMP_ALLOC_LIMBS (nn + 1);
		cy = mpn_lshift (n2p, np, nn, (unsigned int) cnt); // (unsigned int) added by PM
		n2p[nn] = cy;
		nn += adjust;
	    ...
	    if (cnt != 0)
	      mpn_rshift (rp, n2p, dn, (unsigned int) cnt); // (unsigned int)  added by PM
	    else
	      MPN_COPY (rp, n2p, dn);
	    TMP_FREE;
	    return;
