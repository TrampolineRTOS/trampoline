		      ...
		      unsigned int s, mp_ptr ws)
{
  ...
  if( (q & 1) != 0) {
    ASSERT_NOCARRY(mpn_add(ws,ws,n+1,ap+n*((mp_size_t) q),t));
    rp[n] += DO_mpn_addlsh_n(rp, ap+n*((mp_size_t) (q-1)), n, s, rm);
  } else {
    ASSERT_NOCARRY(mpn_add(rp,rp,n+1,ap+n*((mp_size_t) q),t));
  }
  for(i=2; i<q-1; i++)
  {
    rp[n] += DO_mpn_addlsh_n(rp, ap+n*((mp_size_t) i), n, s*(q-i), rm);
    i++;
    ws[n] += DO_mpn_addlsh_n(ws, ap+n*((mp_size_t) i), n, s*(q-i), rm);
  };
