...
mpn_set_str (mp_ptr rp, const unsigned char *str, size_t str_len, int base)
{
  if (POW2_P (base))
    {
	 ...
	 significant character/digit.  */
      ...
      mp_limb_t res_digit;
      mp_size_t size;
      int bits_per_indigit = (int) mp_bases[base].big_base; // (int) added by PM
	  ...
	  if (next_bitpos >= GMP_NUMB_BITS)
	    {
	      ...
	      res_digit = ((mp_limb_t) inp_digit) >> (bits_per_indigit - next_bitpos); // (mp_limb_t) added by PM
	    }
	}
  ...
  if (BELOW_THRESHOLD (str_len, SET_STR_PRECOMPUTE_THRESHOLD))
  ...
  else
    {
      ...
      un = (mp_size_t) (str_len / ((size_t) chars_per_limb) + 1); // (mp_size_t), (size_t) added by PM
...
mpn_set_str_compute_powtab (powers_t *powtab, mp_ptr powtab_mem, mp_size_t un, int base)
{
  ...
  digits_in_base = (size_t) chars_per_limb; // (size_t) added by PM

  p[0] = big_base;
  n = 1;

  count_leading_zeros (i, (UWtype) (un - 1)); // (UWtype) added by PM
  i = GMP_LIMB_BITS - 1 - i;
  ...
  for (pi = i - 1; pi >= 0; pi--)
      ...
      if ((((un - 1) >> pi) & 2) == 0)
	{
	  ...
	  n -= t[n - 1] == 0;
	  digits_in_base -= (unsigned long) chars_per_limb; // (unsigned long) added by PM
	}
#else
...
mpn_bc_set_str (mp_ptr rp, const unsigned char *str, size_t str_len, int base)
{
  ...
  size = 0;
  for (i = (size_t) chars_per_limb; i < str_len; i += (size_t) chars_per_limb) // size_t added by PM
    {
      res_digit = *str++;
      if (base == 10)
      ...
      else
	{
	  for (j = chars_per_limb - 1; j != 0; j--)
	    res_digit = res_digit * ((unsigned long) base) + *str++; // (unsigned long) added by PM
	}
  ...
  big_base = (mp_limb_t) base; // (mp_limb_t) added by PM
  res_digit = *str++;
  if (base == 10)
    { /* This is a common case.
	 Help the compiler to avoid multiplication.  */
      for (j = (long) (str_len - (((long) i) - MP_BASES_CHARS_PER_LIMB_10) - 1); j > 0; j--)
	{
	  res_digit = res_digit * 10 + *str++;
	  big_base *= 10;
	}
    ...
    {
      for (j = ((long) str_len) - (((long) i) - (long) chars_per_limb) - 1; j > 0; j--) // (long) added by PM
	{
	  res_digit = res_digit * (mp_limb_t) base + *str++; // (mp_limb_t) added by PM
	  big_base *= (mp_limb_t) base; // (mp_limb_t) added by PM
	}
    }
